...
test("Test:Functional Programming Coding Challenges from CodeCraft Wiki", () => {
...
water 20`)
    ...
    expect(extractEmailIds(info)).toEqual(["bs@sft.com", "michel@sun.it"])
    ...
    function someUsingReduce(arr, predicate) {
        ...
        return newArr.reduce((result, currentValue) => {
            return predicate(currentValue) || result
        }, false)
    }
    ...
    expect(someUsingReduce([1, 3, 5, 7, 9, 11, 12], (num) => num % 2 === 0)).toBeTruthy()
    expect(someUsingReduce([2, 4, 1, 3, 4, 5, 3, 1], (num) => num % 2 === 0)).toBeTruthy()
    expect(someUsingReduce([10,20,30,33], (num) => num % 2 === 0)).toBeTruthy()
    ...
    function extractAuthorAndTheirQuotes(arr) {
        let newArr = [...arr]
    ...
    expect(extractAuthorAndTheirQuotes(quotes)).toEqual({
        "Thomas Edison": [
    ...
    function allUniqueNutritions(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((result, currentObj) => {
            let resultCopy = [...result]
            return Object.keys(currentObj.nutritions).reduce((nutritions,currentNutrition) => {
                let nutritionsCopy=[...nutritions]
                if (!nutritionsCopy.includes(currentNutrition)) nutritionsCopy.push(currentNutrition)
                return nutritionsCopy
             },resultCopy)
        }, [])
    }

    expect(allUniqueNutritions(fruitAndNut)).toEqual(["protein", "carbs", "sugar", "vitamins"])


    //Get the array of all common health conditions that are treated by  all nuts.

    function commonHealthConditionNutTreats(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.filter(item => item.type === 'nut')
            .map(item => item.treats)
            .reduce((prev, current) => {
                return prev.filter(condition => current.includes(condition));
            });
    }

    expect(commonHealthConditionNutTreats(fruitAndNut)).toEqual(["bp","protein deficiency","skin issues"])

    //Get an array of all unique health conditions that the fruits treat.

    function uniqueHealthConditionFruitTreats(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy
            .filter(currentObj => currentObj.type === 'fruit')
            .reduce((uniqueHealthCondition, currentObj) => {
                let uniqueHealthConditionCopy = [...uniqueHealthCondition]
                return currentObj.treats.reduce((acc, condition) => {
                    if (!commonHealthConditionNutTreats(fruitAndNut).includes(condition)) {
                        acc.push(condition);
                    }
                    return acc
                }, uniqueHealthConditionCopy);
            }, []);
    }


    expect(uniqueHealthConditionFruitTreats(fruitAndNut)).toEqual(["constipation","vitamin deficiency","sleep problems","heart problems","bone issues","migraine"])
    ...
    function addPropertytotalNutrition(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.map((obj) => {
            let totalNutritionCount = Object.values(obj.nutritions).reduce((total, curentNutritionVlaue) => total + curentNutritionVlaue, 0)
            let modifiedObj = { ...obj, "totalNutrition": totalNutritionCount }
            return modifiedObj
        })
    }


    expect(addPropertytotalNutrition(fruitAndNut)).toEqual(
        [
            {
                "name": "Banana",
                "type": "fruit",
    ...
    function totalNutritionValue(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((totalNutritionCount, currentObj) => {
            totalNutritionCount += Object.values(currentObj.nutritions).reduce((total, curentNutritionVlaue) => total + curentNutritionVlaue, 0)
            return totalNutritionCount
        }, 0)
    }

    expect(totalNutritionValue(fruitAndNut)).toBe(577)

    //Which fruits  / nuts solve the bone issues?

    function getfruitOrNutTreatBoneIssue(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((result, currentObj) => {
            let resultCopy = [...result]
            if ([...currentObj.treats].includes("bone issues")) {
                resultCopy.push(currentObj.name)
            }
            return resultCopy
        }, [])
    }
    ...
    expect(getfruitOrNutTreatBoneIssue(fruitAndNut)).toEqual(["Cashew", "Wallnut", "Apple"])


    //Which fruit or nut has maximum nutrition types ( like different type of nutritions)?

    function getMaxNutritionFruitOrNut(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((result, currentObj) => {
            let resultCopy = { ...result }
            const nutritionTypesCount = Object.keys(currentObj.nutritions).length;
            if (nutritionTypesCount >= resultCopy.count) {
                resultCopy.count = nutritionTypesCount
                resultCopy.item.push(currentObj.name)
            }
            return resultCopy;
        }, { count: 0, item: [] }).item;
    }
    ...
    expect(getMaxNutritionFruitOrNut(fruitAndNut)).toEqual(["Banana", "Badam"])


    //Which fruits or nuts solve migraine and have vitamins greater than or equal to 60

    function fruitOrNutSolveMigraine(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((result, currentObj) => {
            let resultCopy = [...result]
            if (Object.values(currentObj.treats).includes("migraine") && currentObj.nutritions["vitamins"] >= 60) {
                resultCopy.push(currentObj.name)
            }
            return resultCopy
        }, [])
    ...
    expect(fruitOrNutSolveMigraine(fruitAndNut)).toEqual(["Apple"])

    //Which fruit or nut has lowest carbs? (Ignore the fruits/nuts that don't have carbs in the first place)


    function lowestCarbFruitOrNut(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy
            .filter(currentObj => currentObj.nutritions?.carbs)
            .reduce((result, currentObj) => {
                let resultCopy = { ...result }
                if (currentObj.nutritions.carbs < resultCopy.carb) {
                    resultCopy.carb = currentObj.nutritions.carbs
                    resultCopy.name = currentObj.name;
                }
                return resultCopy
            }, { carb: Infinity, name: "" }).name
    }


    expect(lowestCarbFruitOrNut(fruitAndNut)).toBe("Badam")
    ...
    function totalProtienIntake(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((result, currentObj) => {
            if (currentObj.type === "nut" && [...currentObj.treats].includes("sugar") && (currentObj.nutritions?.protein)) {
                result += currentObj.nutritions.protein
            }
            return result
        }, 0)
    }
    ...
    expect(totalProtienIntake(fruitAndNut)).toBe(18)
    ...
    function calculateTotalVitaminsWithoutSugarFruits(fruitAndNut) {
        let fruitAndNutCopy = [...fruitAndNut]
        return fruitAndNutCopy.reduce((result, currentObj) => {
            if (currentObj.type === "fruit" && Object.keys(currentObj.nutritions).includes("sugar")) {
                return result
            }
            if (currentObj.nutritions?.vitamins) {
                result += currentObj.nutritions["vitamins"]
            }
            return result
    ...
    expect(calculateTotalVitaminsWithoutSugarFruits(fruitAndNut)).toBe(249)
    ...
    function oddEvenObject(arr) {
        ...
        return numbers.reduce((result, currentNum) => {
            let resultCopy = { ...result }
            currentNum % 2 === 0 ? resultCopy.even.push(currentNum) : resultCopy.odd.push(currentNum)
            return resultCopy
        }, { odd: [], even: [] })
    }
    ...
    function getActors(data) {
        ...
        return movieList.reduce((actorsList, movie) => {
            ...
            return movie.cast.reduce((acc, actor) => {
